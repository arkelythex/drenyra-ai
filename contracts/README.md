...
> [!IMPORTANT]
> **Status: six contracts are FROZEN at v0.1 (release 0.2.0); `brand-system` is DRAFT at v0.2, `connector-adapter` is DRAFT at v0.1, and `projection` is DRAFT at v0.1.** Freezing means the normative surface of each frozen contract is pinned by a conformance suite that runs in CI and fails on drift. `brand-system` ships its conformance suite now and freezes when the ecosystem adopts it (see the [contract](brand-system.md)); `connector-adapter` ships its mock conformance suite now and freezes after adoption plus explicit approval (see the [contract](connector-adapter.md)); `projection` delegates conformance to the existing slice-A suite at `projection/__tests__/` and freezes after documented adoption plus explicit approval (see the [contract](projection.md)).
...
| [projection](projection.md) | 0.1 | DRAFT | Drenyra Command Center, Drenyra Pi, CLI |

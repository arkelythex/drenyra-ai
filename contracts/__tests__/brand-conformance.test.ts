/**
 * brand-system contract conformance (v0.1 DRAFT).
 *
 * Pins the normative surface of contracts/brand-system.md:
 *
 *   1. DERIVATION — every derived token re-derives byte-exact from base tokens
 *      via mix(base, target, ratio) with Math.round per channel.
 *   2. VECTORS — zero tolerance: the drenyra-ai banner SVG only uses canonical
 *      colors; a banned drift color (cyan #22d3ee) fails closed; structural
 *      values (none, url(#...), currentColor) are allowed.
 *   3. RASTER — the built-in PNG decoder validates palette coverage: an
 *      on-token solid PNG passes, an off-palette solid PNG fails below the
 *      required coverage.
 *
 * The checker is driven through its CLI (execFileSync) so the public library
 * surface stays clean. No dependencies: the fixture PNGs are encoded with a
...
interface ConformanceReport {
	...
	version: string;
	status: string;
	derivation: { pass: boolean; violations: string[] };
	assets: AssetResult[];
	pass: boolean;
}
...
const PRIMARY: Rgb = [0x1a, 0x73, 0xe8]; // #1a73e8 — canonical primary
const BANNED_CYAN: Rgb = [0x22, 0xd3, 0xee]; // #22d3ee — documented drift, banned
...
describe("brand-system conformance (v0.1 DRAFT)", () => {
	it("re-derives every derived token byte-exact from base tokens", () => {
		const report = runCli();
		expect(report.contract).toBe("brand-system");
		expect(report.derivation.pass).toBe(true);
		expect(report.derivation.violations).toEqual([]);
	});

	it("passes the drenyra-ai banner SVG with zero tolerance", () => {
		const report = runCli();
	...
	it("fails closed on a banned drift color in an SVG", () => {
		const svg = writeTmp(
			"cyan.svg",
			`<svg xmlns="http://www.w3.org/2000/svg"><rect width="10" height="10" fill="#22d3ee"/></svg>`,
		);
		const report = runCli([svg]);
		const asset = findAsset(report, "cyan.svg");
		expect(asset.pass).toBe(false);
		expect(asset.detail).toContain("#22d3ee");
	});
	...
	it("accepts structural SVG values (none, url(#...), currentColor)", () => {
		const svg = writeTmp(
         ...
         <defs><linearGradient id="g"><stop offset="0%" stop-color="#1a73e8"/></linearGradient></defs>
	...
	it("accepts an on-token solid PNG (palette coverage)", () => {
		const png = writeTmp("on-token.png", solidPng(PRIMARY));
	...
	it("rejects an off-palette solid PNG below required coverage", () => {
		const png = writeTmp("off-palette.png", solidPng(BANNED_CYAN));
		const report = runCli([png]);

...
const SCRIPT = fileURLToPath(new URL("../../scripts/brand-conformance.mjs", import.meta.url));
...
function runCli(extraArgs: string[] = []): ConformanceReport {
  const result = spawnSync(RUNNER, [SCRIPT, "--json", ...extraArgs], { encoding: "utf8" });
  if (result.status === null) {
    throw new Error(`brand-conformance CLI crashed: ${result.error?.message ?? "unknown"}`);
  }
  try {
...
describe("brand-system conformance (v0.2 DRAFT)", () => {
  ...
  it("rejects an off-palette var() declaration inside a theme block", () => {
    ...
    expect(asset.pass).toBe(false);
    expect(Array.isArray(asset.detail) && asset.detail.some((d) => d.includes("#22d3ee"))).toBe(true);
  });
  ...
  it("reports a global FAIL when any asset drifts", () => {
    const svg = writeTmp("drift.svg", `<svg xmlns="http://www.w3.org/2000/svg"><rect fill="#041c78"/></svg>`);
    const report = runCli([svg]);

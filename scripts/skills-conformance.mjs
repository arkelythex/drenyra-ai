...
const MANIFEST_PATH = join(ROOT, "..", "drenyra-skills", "skills", "registry.json");
...
try {
	...
	console.error(`skills-conformance: cannot read ${MANIFEST_PATH}: ${error.message}`);
	process.exit(1);
}
...
for (const id of new Set([...runtimeById.keys(), ...manifestById.keys()])) {
	...
	if (!m) {
		problems.push(`${id}: in runtime BASE_PE_SKILLS but missing from drenyra-skills registry.json`);
		continue;
	}
	if (!runtime) {
		problems.push(`${id}: in drenyra-skills registry.json but missing from runtime BASE_PE_SKILLS`);
		continue;
	}
	for (const field of COMPARE_FIELDS) {
		if (!compare(m[field], runtime[field])) {
			problems.push(`${id}: ${field} differs — manifest ${JSON.stringify(m[field])} vs runtime ${JSON.stringify(runtime[field])}`);
		}
	}
}
...
if (process.argv.includes("--json")) {
	process.stdout.write(`${JSON.stringify({ contract: "skills-registry", pass, problems }, null, 2)}\n`);
} else {
	process.stdout.write(`skills-registry conformance (manifest → runtime)\n`);
	if (pass) {
		process.stdout.write(`✓ ${manifest.skills.length} skill definitions in sync with BASE_PE_SKILLS\n`);
	} else {

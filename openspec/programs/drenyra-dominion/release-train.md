...
## 4.1 Deterministic lock checksums and the Phase B5 attestation

A promoted lock carries a structured `checksums` block generated by
`scripts/checksum-lock.mjs` (SDD-010):
...
node scripts/checksum-lock.mjs \
  ...
  [--artifact <sibling-repo>=<immutable-artifact> ...] \
  [--output <staging-file>]
```

The producer hashes each supplied artifact with SHA-256, orders entries
canonically (Unicode code-point by `repository`, then `artifact`), and derives
`setSha256` over the compact serialized entries. Generation is deterministic:
identical lock facts and artifact bytes produce byte-identical output from any
working directory. `--verify` re-derives the block from the same inputs and
exits non-zero on any difference without emitting replacement data.

Rules enforced in tooling:

- The lock file itself is never a checksum input (checksum self-inclusion rule).
- No HEAD/tag-target/carrying-commit SHA is ever inspected or emitted (bootstrap
  rule); the script never invokes Git or GitHub.
- Unknown/awaiting-evidence repositories never receive invented checksum
  material; only the host artifact and evidenced current-claim siblings are
  admissible inputs.

Publication is a **two-step gate**. Commit B sets `status: "promoted"` in the
lock only after every local gate passes — fresh revision-bound host
verification, resolved sibling facts, populated deterministic checksums,
resolvable evidence IDs, and schema validity. That committed checkpoint is a
local gate only: publication completes when the external Phase B5 attestation —
a signed annotated tag plus a GitHub Release asset
(`drenyra-dominion-v0.4.0.attestation.json`) — is created over commit B and read
back successfully against the exact lock bytes, checksum set, and verified
revision. A failing B5 readback means the checkpoint is not published and

...
| --- | --- | --- | --- | --- | --- |
| PostgreSQL production adapter | ✅ | `missions/store.postgres.ts` + **real integration suite** (5 tests against PostgreSQL 18, `created_at` fix); unit tests green | JSON file store is dev-only | Run in CI with a Postgres service | v0.3 |
| MCP server | ✅ | `mcp/` — JSON-RPC 2.0 server + `drenyra-ai mcp serve` stdio binding (smoke-tested end-to-end); 10 tests | External hosts need a uniform protocol | Agent-host adapters consume the MCP surface | v0.3 |
...
| `install` / `doctor` / `sync` | ✅ | `doctor` (read-only), `install` (detects hosts, writes markers + managed skills asset), `sync` (preserves foreign changes); 7 tests | Gentle-AI-equivalent configurator experience | Registry dependency when published | v0.4 |
| Versioned skills (registry + Peru base) | ✅ | `skills/` — SkillRegistry + 6 PE skills (IGV, SIRE compare, detractions, retentions, perceptions, SIRE filing); 12 tests | Unversioned policy changes break reproducibility | Skills surfaced through `capabilities` and shipped by `install` | v0.3 |
...
| SDK + governance documentation | ✅ | `docs/sdk.md` (16 subpaths with examples) + `docs/governance.md` (open-core governance: RFC review, CODEOWNERS, DCO, compatibility, certified connectors) — v1.0 criterion 12 | Undocumented public surface | Reviewed by an external integrator | v1.0 |
| External reconciliation (UNKNOWN states) | ✅ | `missions/reconciliation.ts` + `MissionRuntime.reconcile()` (UNKNOWN → RUNNING on record/retry, stays UNKNOWN on human; RECONCILED event); 14 tests | Blind retries duplicate postings | Adapter calls route through runtime.reconcile | v0.5 |

 ...
 */
/**
 * Module-boundary test — `igv/` is a pure library module: every import must
 * resolve within the module itself (or `node:` builtins), never to a project
 * module, and never to `agents/`, `cmd/`, `ledger/`, `mcp/`, `adapters/`, or
 * any sibling engine module. The audit-only `ledger/` module must never
 * import it.
 */
...
describe("igv module boundary", () => {
	...
	it("is never imported by the audit-only ledger module", () => {
		...
		const violations = ledgerFiles.filter((file) =>
			readFileSync(file, "utf8").includes("igv/"),
		);
		expect(violations).toEqual([]);
	});
});

...
import { computeSkillChecksum, isSkillInForce, SkillError, SkillRegistry } from "../index.js";
...
describe("SkillRegistry", () => {
	...
	it("rejects invalid definitions (bad id, version, jurisdiction, tier, permission)", () => {
		const registry = new SkillRegistry();
		expect(() => registry.register(baseSkill({ id: "INVALID ID" }))).toThrow(SkillError);
		expect(() => registry.register(baseSkill({ version: "1" }))).toThrow(SkillError);
		expect(() => registry.register(baseSkill({ jurisdiction: "PERU" }))).toThrow(SkillError);
		expect(() => registry.register(baseSkill({ maxAutonomy: "R9" as never }))).toThrow(SkillError);
		expect(() => registry.register(baseSkill({ requiredPermissions: ["root:all"] }))).toThrow(SkillError);
		expect(() => registry.register(baseSkill({ validity: { from: "2026-13-01" } }))).toThrow(SkillError);
	});

	it("resolves the skill in force at a date, honoring the exclusive `to`", () => {
		const registry = new SkillRegistry();
		const skill = baseSkill({ validity: { from: "2026-01-01", to: "2026-12-31" } });
		registry.register(skill);
	...
	it("throws SKILL_OUT_OF_VALIDITY when nothing is in force at a date", () => {
		const registry = new SkillRegistry();
		registry.register(baseSkill({ validity: { from: "2026-01-01", to: "2026-06-30" } }));
		try {
	...
	it("returns the latest in-force version at a date", () => {
		const registry = new SkillRegistry();
		registry.register(baseSkill({ version: "1.0.0", validity: { from: "2026-01-01" } }));
		registry.register(baseSkill({ version: "1.1.0", validity: { from: "2026-07-01" } }));
		expect(registry.resolveAt("pe.igv-validate", "2026-03-01").version).toBe("1.0.0");
		expect(registry.resolveAt("pe.igv-validate", "2026-08-01").version).toBe("1.1.0");
	});
	...
	it("does not retroactively change a historical resolution", () => {
		...
		registry.register(baseSkill({ version: "2.0.0", validity: { from: "2026-09-01" } }));
		expect(registry.resolveAt("pe.igv-validate", "2026-03-01")).toEqual(historical);
		expect(registry.resolveAt("pe.igv-validate", "2026-10-01").version).toBe("2.0.0");
	});
	...
	it("filters by jurisdiction and enforces jurisdiction mismatch", () => {
		...
		expect(() => registry.resolveAt("pe.igv-validate", "2026-06-01", "CL")).toThrow(SkillError);
	});

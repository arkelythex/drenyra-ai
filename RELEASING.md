...
- [Commit and release discipline](#commit-and-release-discipline)

## Version policy

- Until the first contract is frozen, releases use **`0.0.1-prealpha.x`** (x increments per release).
- The first release that freezes a contract (`mission-protocol`, `candidate`, `receipt`, `gate` — per the [ROADMAP](ROADMAP.md)'s Phase 1) is **`0.1.0`**.
...
> [!NOTE]
> **Pre-alpha:** provenance items beyond the signed tag and changelog are documented as the target; the SBOM/manifest tooling lands with the first real release.
...
- See what shipped in each version → [CHANGELOG.md](CHANGELOG.md)

...
describe("McpServer", () => {
	...
	it("calls the capabilities tool", async () => {
		const server = makeServer();
		const response = await server.handleMessage(
			JSON.stringify({ jsonrpc: "2.0", id: 3, method: "tools/call", params: { name: "capabilities", arguments: {} } }),
		);
		const parsed = JSON.parse(response!) as { result: { content: Array<{ text: string }> } };
		const content = JSON.parse(parsed.result.content[0]!.text) as { contracts: unknown[]; jurisdictions: string[] };
	...
	it("surfaces tool handler errors as INTERNAL_ERROR", async () => {
		const server = makeServer();
		const response = await server.handleMessage(
			JSON.stringify({ jsonrpc: "2.0", id: 5, method: "tools/call", params: { name: "ledger.validate", arguments: { ledger: "not-an-array" } } }),
		);
